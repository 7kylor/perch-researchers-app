...
export class AcademicDatabaseService {
    ...
    try {
      ...
      for (const result of Array.from(results).slice(0, limit)) {
        ...
        if (titleElement) {
          ...
          const year = yearMatch && yearMatch[1] ? parseInt(yearMatch[1]) : undefined;
  ...
  async searchSemanticScholar(query: string, limit: number = 20): Promise<SearchResult> {
    ...
    try {
      ...
      return {
        papers,
        totalResults: (data as any).total || papers.length,
        searchTime: Date.now() - startTime,
      };
    } catch (error) {
  ...
  async searchPubMed(query: string, limit: number = 20): Promise<SearchResult> {
    ...
    try {
      ...
      const papers: AcademicPaper[] = [];
      const results = (summaryData as any).result || {};
      ...
      return {
        papers,
        totalResults: parseInt((searchData as any).esearchresult?.count || '0'),
        searchTime: Date.now() - startTime,
      };
    } catch (error) {
  ...
  async getPaperByDOI(doi: string): Promise<AcademicPaper | null> {
    try {
      ...
      if (semanticResponse.ok) {
        const data = await semanticResponse.json() as any;
        return {
          title: data.title,
